...
#include "temp/graphics_old/d3d11/vertex_shader_impl_d3d11.h"
...
VertexShader::SPtr VertexShader::create(NativeHandle  device,
    ...
    struct Creator : public VertexShader {
        Creator(NativeHandle device, const String& source, Bool is_binary)
            : VertexShader(device, source, is_binary) {}
    };
    auto p = std::make_shared<Creator>(device, source, is_binary);
    return std::move(p);
}

VertexShader::VertexShader(NativeHandle device, const String& source,
                           Bool is_binary) {
