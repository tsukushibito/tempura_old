...
Context::Impl::Impl(NativeHandle& device, Context& context)
    : context_(context), command_(new opengl::Command) {
    (void)device;
}
...
void Context::Impl::setDepthStencileState(
    const DepthStencileStateSPtr& depth_stencile_state) {
    command_->depth_stencile_state = depth_stencile_state.get();
}

void Context::Impl::setRasterizerState(
    const RasterizerStateSPtr& rasterizer_state) {
...
void Context::Impl::setVertexBuffer(UInt32                  slotNum,
...
void Context::Impl::setConstantBuffer(
    const String& name, const ConstantBufferSPtr& constant_buffer) {}
...
void Context::Impl::drawIndexed(UInt32 index_count, UInt32 start_index_location,
