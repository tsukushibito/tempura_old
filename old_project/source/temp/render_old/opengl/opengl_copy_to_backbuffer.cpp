...
CopyToBackBuffer::CopyToBackBuffer(const GraphicsDeviceSPtr& graphics_device,
    ...
    auto vs = resource_old::VertexShader::create(vs_path);
    vs->load();

    Path ps_path = shader_directory_ + kCopyPS;

    auto ps = resource_old::PixelShader::create(ps_path);
    ps->load();
...
GLuint CopyToBackBuffer::arrayBuffer(temp::graphics_::VertexAttribute attribute) {
