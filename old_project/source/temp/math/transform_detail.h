...
template <typename T>
TransformBase<T>::TransformBase(const Vec3& pos, const Quat& rot,
                                const Vec3& scale)
...
typename TransformBase<T>::Mat44 TransformBase<T>::toMatrix() const {
    ...
    Mat44 scale(ColOrder(), Vec4(scale_.x(), 0, 0, 0),
                Vec4(0, scale_.y(), 0, 0), Vec4(0, 0, scale_.z(), 0),
                Vec4(0, 0, 0, 1));
...
template <typename T>
TransformBase<T> TransformBase<T>::operator*(
    const TransformBase<T>& rhs) const {
    return combine(rhs);
}

template <typename T>
TransformBase<T> TransformBase<T>::combine(const TransformBase<T>& rhs) const {
    return TransformBase<T>(Vec3(position_) + rhs.position_,
                            rotation_ * rhs.rotation_, scale_ * rhs.scale_);
}
