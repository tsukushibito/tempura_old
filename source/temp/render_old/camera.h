...
#include "temp/graphics_/render_target.h"
...
public:
    using RenderTarget     = temp::graphics_::RenderTarget;
    using RenderTargetSPtr = temp::graphics_::RenderTargetSPtr;

private:
    Camera(const RenderTargetSPtr&             render_target,
           const std::function<void(Camera*)>& on_destroy)
        : on_destroy_(on_destroy), render_target_(render_target) {}

public:
    ~Camera() { on_destroy_(this); }

    const RenderTargetSPtr& renderTarget() const { return render_target_; }
