...
GLuint createOpenglPixelShaderFromSource(const String& source) {
    ...
    GLuint pixel_shader
        = glCallWithErrorCheck(glCreateShader, GL_FRAGMENT_SHADER);
    const GLchar* src_string = source.c_str();
    const GLint   length     = static_cast<GLint>(source.size());
    glCallWithErrorCheck(glShaderSource, pixel_shader, (GLsizei)1, &src_string,
                         &length);
...
GLuint createOpenglPixelShaderFromBinary(const String& source) { return 0; }
}


PixelShader::Impl::Impl(NativeHandle device, PixelShader& pixel_shader,
                        const String& source, Bool is_binary)
    : pixel_shader_(pixel_shader) {
    if (is_binary) {
        pixel_shader_.native_handle_.value
            = createOpenglPixelShaderFromBinary(source);
    } else {
        pixel_shader_.native_handle_.value
