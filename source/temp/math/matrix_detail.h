...
template <typename T>
Matrix44Base<T>::Matrix44Base(RowOrder, const Vec4& row0, const Vec4& row1,
...
template <typename T>
Matrix44Base<T>::Matrix44Base(ColOrder, const Vec4& col0, const Vec4& col1,
...
Bool Matrix44Base<T>::operator==(const Matrix44Base& rhs) {
    return (cols_[0] == rhs.cols_[0] && cols_[1] == rhs.cols_[1]
            && cols_[2] == rhs.cols_[2] && cols_[3] == rhs.cols_[3]);
}
...
template <typename T>
Vector4Base<T> operator*(const Vector4Base<T>&  lhs,
...
Matrix44Base<T> operator*(const Matrix44Base<T>& lhs,
    ...
    for (Int32 row = 0; row < 4; ++row) {
        for (Int32 col = 0; col < 4; ++col) {
            retVal.cols_[col][row]
                = Vector4Base<T>::dot(lhs.row(row), rhs.cols_[col]);
        }
    }
...
#else
    const auto& m   = cols_;
    Float32     det = (m[0][0] * m[1][1] * m[2][2] * m[3][3])
...
Vector4Base<T> Matrix44Base<T>::row(Size index) const {
    ...
    return Vector4Base<T>(cols_[0][index], cols_[1][index], cols_[2][index],
                          cols_[3][index]);
}
...
const Matrix44Base<T> Matrix44Base<T>::kIdentity
    = Matrix44Base<T>(ColOrder(), Vec4(1.0f, 0.0f, 0.0f, 0.0f),
