...
template <typename T>
QuaternionBase<T>::QuaternionBase(T x, T y, T z, T w) : vec4_(x, y, z, w) {}
...
template <typename T>
const T& QuaternionBase<T>::x() const {
    return vec4_[0];
}
...
template <typename T>
const T& QuaternionBase<T>::y() const {
    return vec4_[1];
}
...
template <typename T>
const T& QuaternionBase<T>::z() const {
    return vec4_[2];
}
...
template <typename T>
const T& QuaternionBase<T>::w() const {
    return vec4_[3];
}

template <typename T>
T& QuaternionBase<T>::operator[](Size index) {
    TEMP_ASSERT(index <= 3, "");
    return vec4_[index];
}

template <typename T>
const T& QuaternionBase<T>::operator[](Size index) const {
    TEMP_ASSERT(index <= 3, "");
    return vec4_[index];
}
...
template <typename T>
Vector3Base<T> QuaternionBase<T>::rotateVector3(
    const Vector3Base<T>& rhs) const {
    auto quat    = *this;
...
QuaternionBase<T> operator*(const QuaternionBase<T>& lhs,
    ...
    return QuaternionBase<T>(lhs.w() * rhs.x() + lhs.x() * rhs.w()
...
QuaternionBase<T> operator*(Float32 lhs, const QuaternionBase<T>& rhs) {
    return QuaternionBase<T>(lhs * rhs.x(), lhs * rhs.y(), lhs * rhs.z(),
                             lhs * rhs.w());
}
...
template <typename T>
const QuaternionBase<T> QuaternionBase<T>::kIdentity
    = QuaternionBase<T>((T)0, (T)0, (T)0, (T)1);
}
}
