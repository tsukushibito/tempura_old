...
DepthStencileState::SPtr DepthStencileState::create(NativeHandle device, DepthStencileFunc depth_func, DepthStencileFunc stencile_func) {
    struct Creator : public DepthStencileState {
        Creator(NativeHandle device, DepthStencileFunc depth_func, DepthStencileFunc stencile_func) : DepthStencileState(device, depth_func, stencile_func) {}
    };
    auto p = std::make_shared< Creator >(device, depth_func, stencile_func);
    return std::move(p);
}

DepthStencileState::DepthStencileState(NativeHandle device, DepthStencileFunc depth_func, DepthStencileFunc stencile_func) {
