...
RasterizerState::SPtr RasterizerState::create(NativeHandle device, const RasterizerDesc &desc) {
    struct Creator : public RasterizerState {
        Creator(NativeHandle device, const RasterizerDesc &desc) : RasterizerState(device, desc) {}
    };
    auto p = std::make_shared< Creator >(device, desc);
    return std::move(p);
}

RasterizerState::RasterizerState(NativeHandle device, const RasterizerDesc &desc) {
